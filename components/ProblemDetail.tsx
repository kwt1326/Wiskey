...
export function ProblemDetail({ bountyId }: ProblemDetailProps) {
  ...
  const isOwner = userWallet === bounty.postedBy;
  const hasUserAnswered = bounty.answers.some(a => a.responderWallet === userWallet);
  const isCompleted = bounty.status === 'completed';

  const handleSubmitAnswer = async () => {
    if (!answerContent.trim() || !isWalletConnected) return;
  ...
  return (
    <div className="flex flex-1 flex-col min-h-0 w-full">
      ...
      <div className="flex-1 px-5 py-6 space-y-6 overflow-y-auto min-h-0" style={{ maxHeight: 'calc(100vh - 77px)' }}>
        ...
        {!isCompleted && isWalletConnected && !hasUserAnswered && !isOwner && (
