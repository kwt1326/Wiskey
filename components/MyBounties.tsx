...
export function MyBounties({ bounties, userWallet, onNavigate }: MyBountiesProps) {
  ...
  const renderBountyCard = (bounty: Bounty, type: 'posted' | 'answered') => {
    ...
    return (
      <Card
        key={bounty.id}
        className="bg-white/80 backdrop-blur-sm border border-slate-200/50 shadow-sm hover:shadow-md transition-all cursor-pointer"
  ...
  const renderEmptyState = (type: 'posted' | 'answered') => (
    <div className="text-center py-12">
      ...
      <Button
        onClick={() => onNavigate(type === 'posted' ? 'post-problem' : 'home')}
    ...
    <div className="h-full flex flex-col bg-gradient-to-br from-slate-50 to-slate-100">
      ...
      <div className="flex-1 px-5 py-6 overflow-y-auto">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full h-full flex flex-col">
          <TabsList className="grid w-full grid-cols-2 bg-white/70 backdrop-blur-sm border border-slate-200/50 rounded-2xl p-2 h-14">
            <TabsTrigger 
              value="posted" 
          ...
          <TabsContent value="posted" className="flex-1 mt-6 pb-32">
          ...
          <TabsContent value="answered" className="flex-1 mt-6 pb-32">
            <div className="space-y-4 h-full">
      ...
      <div className="bg-white/95 backdrop-blur-sm border-t border-slate-200/50">
        {/* Stats Summary */}
                ...
                <div className="grid grid-cols-3 gap-4 text-center">
                  <div>
                    <div className="text-xl font-bold text-emerald-600">
                      {postedBounties.reduce((sum, b) => sum + b.reward, 0).toFixed(3)}
                    </div>
                    <div className="text-sm text-slate-500 font-medium">ETH Posted</div>
                  </div>
                  <div>
                    <div className="text-xl font-bold text-amber-600">
                      {answeredBounties.filter(b => getUserAnswerStatus(b) === 'winner').length}
                    </div>
                    <div className="text-sm text-slate-500 font-medium">Wins</div>
                  </div>
                  <div>
                    <div className="text-xl font-bold text-blue-600">
                      {answeredBounties.filter(b => getUserAnswerStatus(b) === 'winner')
                        .reduce((sum, b) => sum + b.reward, 0).toFixed(3)}
                    </div>
                    <div className="text-sm text-slate-500 font-medium">ETH Earned</div>
                  </div>
                </div>
              </CardContent>
            </Card>
        ...
        <BottomNavigation currentScreen="my-bounties" onNavigate={onNavigate} />
      </div>
    </div>
  );
}
