...
import { Settings, Copy, FileText, MessageSquare, Award, CheckCircle, Clock, TrendingUp } from 'lucide-react';
import type { Screen, Bounty } from '@/components/types';
...
export function Profile({ isWalletConnected, userWallet, bounties, onNavigate, onShowConnectModal }: ProfileProps) {
  const copyAddress = () => {
    if (userWallet) {
        ...
        {!isWalletConnected ? (
          ...
          <>
                ...
                {recentActivity.slice(0, 5).map((activity, index) => (
                  <div key={index} className="flex items-center space-x-3 p-3 bg-emerald-50/50 rounded-2xl">
                    <div className={`w-8 h-8 rounded-xl flex items-center justify-center ${
                      activity.type === 'posted' ? 'bg-emerald-100' :
                      activity.type === 'answered' ? 'bg-teal-100' : 'bg-amber-100'
                    }`}>
                      {activity.type === 'posted' ? (
                        <FileText className={`h-4 w-4 ${
                          activity.type === 'posted' ? 'text-emerald-600' : ''
                        }`} />
                      ...
                      <div className="flex items-center space-x-2 mt-1">
                        <Badge 
                          ...
                          className={`text-xs px-2 py-0.5 ${
                            activity.status === 'Open' ? 'bg-emerald-100 text-emerald-700' :
            ...
            <div className="space-y-4 pb-6">
              <Button
                onClick={() => onNavigate('my-bounties')}
      ...
      <div className="bg-white/95 backdrop-blur-sm border-t border-emerald-200/50">
        <div className="flex items-center justify-center py-3">
          <div className="flex items-center space-x-2 text-sm text-slate-500">
            ...
            <span>Powered by Base · v0.1</span>
          </div>
        </div>
